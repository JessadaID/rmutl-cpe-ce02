<script lang="ts">
  ...
  let confirmModalConfirmButtonClass =
  ...
  function openTermModalForEdit(term) {
    ...
    currentProjectLimit =
      term.projectLimit === undefined ? 5 : term.projectLimit; // Default to 5 if not set
    currentDirectorScoreLimit =
      term.directorScoreLimit === undefined ? 0 : term.directorScoreLimit; // Default to 0 if not set
    adviserScoreLimitValue =
      term.adviserScoreLimit === undefined ? 0 : term.adviserScoreLimit; // Default to 0 if not set
    subjectScoreLimitValue =
      term.subjectScoreLimit === undefined ? 0 : term.subjectScoreLimit; // Default to 0 if not set
  ...
  async function processUpdateTerm(
    updatedName: string,
    projectLimit: number | null,
    directorScoreLimit: number | null,
    adviserScoreLimitValue: number | null = 0,
    subjectScoreLimitValue: number | null = 0
  ) {
    ...
    try {
      ...
      await updateDoc(termRef, {
        ...
        directorScoreLimit:
          directorScoreLimit === null ? 0 : Number(directorScoreLimit),
        adviserScoreLimit:
          adviserScoreLimitValue === null ? 0 : Number(adviserScoreLimitValue),
        subjectScoreLimit:
          subjectScoreLimitValue === null ? 0 : Number(subjectScoreLimitValue),
        updatedAt: serverTimestamp(),
      });
      await loadTerms();
  ...
  async function processCreateTerm(
    newTermName: string,
    projectLimit: number | null,
    directorScoreLimit: number | null,
    adviserScoreLimitValue: number | null = 0,
  ...
  async function copyTasksFromLatestTerm(newTermName: string) {
    try {
      ...
      if (
        latestTermData.error ||
        !latestTermData.data ||
        latestTermData.data.length < 2
      ) {
      ...
      const tasksResponse = await fetch(
        `/api/tasks-data?term=${encodeURIComponent(previousTerm.term)}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  function handleDeleteRequest() {
    // Renamed to avoid conflict if needed, called by Modal's delete event
    ...
    confirmModalOnConfirm = async () => {
      ...
      try {
        ...
        await DeleteTaskTermname(currentEditingTerm.term); // ลบงานที่เกี่ยวข้องกับเทอมนี้
        await loadTerms();
  ...
  // ลบงานที่เกี่ยวข้องกับเทอมนี้
  async function DeleteTaskTermname(termname) {
    try {
      const response = await fetch(`/api/tasks-data`, {
        method: "DELETE",
        headers: {
      ...
      const result = await response.json();
      if (result.error) {
        console.error("Error deleting tasks for term:", result.error);
        dangerToast("เกิดข้อผิดพลาดในการลบงานสำหรับเทอมนี้" + result.error);
      } else {
        //console.log(`งานที่เกี่ยวข้องกับเทอม "${termname}" ถูกลบเรียบร้อยแล้ว`);
      }
    } catch (error) {
      console.error("Error deleting tasks for term:", error);
      dangerToast("เกิดข้อผิดพลาดในการลบงานสำหรับเทอมนี้" + error.message);
    }
  }

  // Updated to handle an object from the modal's save event
  async function handleModalSave(eventDetail: {
    termName: string;
    projectLimit: number | null;
    directorScoreLimit: number | null;
    adviserScoreLimit: number | null;
    subjectScoreLimit: number | null;
  }) {
    let success = false;
    if (modalMode === "create") {
      success = await processCreateTerm(
        eventDetail.termName,
        eventDetail.projectLimit,
        eventDetail.directorScoreLimit,
        eventDetail.adviserScoreLimit,
        eventDetail.subjectScoreLimit
      );
    } else if (modalMode === "edit") {
      success = await processUpdateTerm(
        eventDetail.termName,
        eventDetail.projectLimit,
        eventDetail.directorScoreLimit,
        eventDetail.adviserScoreLimit,
    ...
    if (success) {
      closeTermModal();
      successToast(
        `เทอม "${eventDetail.termName}" ได้รับการ ${modalMode === "create" ? "สร้าง" : "แก้ไข"} สำเร็จ`
      );
    }
  }
        ...
        <div class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
          ...
          {#if term.createdAt || term.updatedAt}
            ...
            <p class="text-sm text-gray-600 mt-1">
              จำนวนโครงงานที่รับได้ (ของที่ปรึกษา): {term.projectLimit !==
              undefined
                ? term.projectLimit
                : "ไม่ได้กำหนด"}
            </p>
            <p class="text-sm text-gray-600 mt-1">
              คะแนน (กรรมการ: {term.directorScoreLimit !== undefined
                ? term.directorScoreLimit
                : "ไม่ได้กำหนด"} % , ที่ปรึกษา: {term.adviserScoreLimit !==
              undefined
                ? term.adviserScoreLimit
                : "ไม่ได้กำหนด"} % , อาจารย์ประจำวิชา: {term.subjectScoreLimit !==
              undefined
                ? term.subjectScoreLimit
                : "ไม่ได้กำหนด"} %)
            </p>
          {/if}
        </div>
...
<Modal
  ...
  bind:adviserScoreLimitValue
  bind:subjectScoreLimitValue
  bind:isCheckedLastermTask
  {loading}
