<script>
  ...
  export let data; // Export data prop for parent component to pass data
  $: termId = $page.params.term; // Reactive assignment to get term from page params
  onMount(async () => {
    allProjects = data?.terms || []; // Use data prop to initialize allProjects
    ...
    /* 
		const { termId } = $page.params; // Get the term from the page parameters
        console.log('Term ID from page params:', termId);
		try {
  ...
  $: if (allProjects) {
    ...
    if (!lowerSearchTerm) {
      ...
      filteredProjects = baseList.filter((project) => {
        const nameThMatch = project.project_name_th
          ?.toLowerCase()
          .includes(lowerSearchTerm);
        const nameEnMatch = project.project_name_en
          ?.toLowerCase()
          .includes(lowerSearchTerm);
        const memberMatch = project.members?.some((member) =>
          member?.toLowerCase().includes(lowerSearchTerm)
        );
  ...
  async function viewProjectDetails(projectId) {
    if (checkAuthStatus()) {
      goto(`/cpe02/data/term/${termId}/project-detail/${projectId}`);
    } else {
...
<div class="container mx-auto px-4 py-8">
  <h1 class="text-2xl md:text-3xl font-bold text-center text-gray-800 mb-6">
    โครงงานสำหรับภาคเรียน: <span class="text-indigo-600"
      >{termId || "ไม่ระบุ"}</span
    >
  </h1>
              ...
              <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                ...
                <td class="px-6 py-4 whitespace-normal">
                  {#if project.members && project.members.length > 0}
                    <div
                      class="text-sm text-gray-700 flex items-center space-x-1"
                    >
          ...
          {:else}
            ...
            <tr>
              <td colspan="6" class="px-6 py-10 text-center text-gray-500">
                {#if filterByMyProjects && userEmail}
                  {#if searchTerm}
                    ไม่พบโครงงานที่คุณเป็นที่ปรึกษา ({userEmail})
                    ที่ตรงกับคำค้นหา "{searchTerm}"
                  {:else}
