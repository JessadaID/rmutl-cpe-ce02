            ...
            const deletePromises = projectData.images
              ...
              .map(async (image) => {
                try {
                  ...
                  warningToast(
                    `ไม่สามารถลบรูปภาพบางส่วนได้: ${image.name || "รูปภาพ"}`
                  );
                }
              });
...
{:else if project}
  <div class="mx-auto w-full min-h-screen bg-gray-100 px-4 pb-20 pt-6">
    <!-- Main Content Grid -->
    <div class="md:flex md:gap-8">
      ...
      <div
        class="{showTask
          ? 'md:w-7/12 lg:w-8/12'
          : 'md:w-10/12 lg:w-8/12 mx-auto'} mb-8 md:mb-0"
      >
        <!-- Main Navigation Tabs -->
        <div class="sticky top-16 z-10 rounded-t-lg bg-white px-6 py-0 shadow-md">
          <div
            class="flex items-center overflow-x-auto overflow-y-hidden border-b border-gray-200"
          >
            <div>
              <button
                on:click={() => selectVersion("current")}
                class="py-6 px-6 -mb-px font-medium text-sm focus:outline-none whitespace-nowrap {activeMainView ===
                'current'
                  ? 'border-b-2 border-indigo-500 text-indigo-600'
                  : 'text-gray-500 hover:text-indigo-500 border-b-2 border-transparent hover:border-gray-300'}"
              >
                <svg
              ...
              <button
                ...
                'previous_version'
                  ? 'border-b-2 border-indigo-500 text-indigo-600'
                  : 'text-gray-500 hover:text-indigo-500 border-b-2 border-transparent hover:border-gray-300'} {projectVersions.length ===
                0
            ...
            <div class="ml-auto">
              ...
              {#if can_edit && activeMainView === "current"}
                <div class="my-4 flex justify-end gap-3">
        ...
        <div class="h-full rounded-b-lg bg-white p-4 shadow-lg sm:p-6 md:p-8">
          ...
          <div class="mb-6 pb-4 border-b border-gray-200 relative">
            ...
            <p class="text-lg text-gray-600 break-all">
            ...
            <div>
              ...
              <ul class="list-disc list-inside pl-4 space-y-1 text-gray-600">
                ...
                {#if project.External_consultant}
                  <li class="mt-1 italic">
                    {project.External_consultant} (ที่ปรึกษาภายนอก)
                  </li>
                {/if}
              </ul>
            </div>
          ...
          <div class="space-y-6">
            <div>
              ...
              <p class="text-gray-700 leading-relaxed whitespace-pre-wrap">
                {project.project_problem || "ไม่มีข้อมูล"}
              </p>
            </div>
            <div>
              <h2 class="text-lg font-semibold text-gray-700 mb-2">
              ...
              <p class="text-gray-700 leading-relaxed whitespace-pre-wrap">
                {project.project_Objective || "ไม่มีข้อมูล"}
              </p>
            </div>
            <div>
              <h2 class="text-lg font-semibold text-gray-700 mb-2">
              ...
              <p class="text-gray-700 leading-relaxed whitespace-pre-wrap">
                {project.research_data || "ไม่มีข้อมูล"}
              </p>
            </div>
            <div>
              <h2 class="text-lg font-semibold text-gray-700 mb-2">
              ...
              <p class="text-gray-700 leading-relaxed whitespace-pre-wrap">
                {project.Theory_principles || "ไม่มีข้อมูล"}
              </p>
            </div>
                ...
                >
                  {#each project.images as image}
                    ...
                    <div class="text-center group">
                      ...
                      />
                      <p class="text-xs text-gray-600 truncate px-1">
            ...
            {#if project.Operation_Schedule}
              <div>
                <h2 class="text-lg font-semibold text-gray-700 mb-3">
                  9. แผนและระยะเวลาดำเนินงาน ({project.Operation_Schedule
                    .tableTitle || "Timeline"})
                </h2>
                <div
                  class="overflow-x-auto border border-gray-300 rounded-md shadow-sm"
                >
                  <table class="w-full min-w-[600px] border-collapse text-sm">
                    <thead>
                      <tr class="bg-gray-100 text-left">
                        <th
                          class="border-b border-gray-300 p-3 font-semibold text-gray-700 w-1/3 sm:w-1/4"
                          >กิจกรรม</th
                        >
                        {#each project.Operation_Schedule.monthLabels as month}
                          <th
                            class="border-b border-l border-gray-300 p-3 font-semibold text-gray-700 text-center whitespace-nowrap"
                            >{month}</th
                          >
                        {/each}
                      </tr>
                    ...
                    <tbody class="bg-white">
                      {#each project.Operation_Schedule.activities as activity (activity.id)}
                        ...
                        >
                          <td
                            class="border-b border-gray-300 p-3 text-gray-700 align-top"
                          >
                            {activity.name}
                          </td>
                          ...
                          {#each project.Operation_Schedule.monthLabels as month, i}
                            <td
                              class="border-b border-l border-gray-300 p-0 text-center h-full"
                            >
                              <div
                                class="h-full w-full py-3"
                                style:background-color={activity.months[i]
                                  ? activity.color
                                  : "transparent"}
                              >
                                &nbsp;
                ...
                {#each project.budgetItems as bugget, i}
                  ...
                  >
                    <span class="text-gray-700"
                      >{i + 1}. {bugget.description}</span
                    >
                    <span class="text-gray-700"
                      >{bugget.amount.toLocaleString("th-TH", {
                ...
                <div class="px-4 pt-6 pb-0 sm:px-6">
                  <div class="flex border-b border-gray-200">
                    <button
                      ...
                      class="py-3 px-4 -mb-px font-medium text-sm focus:outline-none whitespace-nowrap {activeTaskView ===
                      'process'
                        ? 'border-b-2 border-indigo-500 text-indigo-600'
                        : 'text-gray-500 hover:text-indigo-500 border-b-2 border-transparent hover:border-gray-300'}"
                    >
                ...
                <div class="p-6">
                  {#if activeTaskView === "process"}
                    <Process
                      {project}
                      isLoading={false}
                      can_edit_task={activeMainView === "current"
                        ? can_edit_tasks
                        : false}
                      Task={termTasks}
                      isLoadingtext={false}
                  ...
                  {:else if activeTaskView === "scores"}
                    <ProjectScore {project} {form_data} />
                  {:else if activeTaskView === "exam"}
                    <StudentSelect {projectId} />
                  {/if}
                </div>
              </div>
...
{#if showModal && selectedImage}
  ...
  >
    ...
    >
      ...
      />
      <p
        class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-60 text-white text-center p-2 text-sm"
      >
